...
Present and contract-tested: `sys.version`, `sys.capabilities`, `sys.plugins`, `prefs.get`, `prefs.set`, `project.create|open|close|save|compile|reload|props|convert`, `entry.list|get|set`, `matches.query`, `glossary.query|add`, `search.run`, `search.replace`, `stats.get`, `issues.list`, `filters.list`, `filters.options`, `mt.query`, `dict.query`, `completer.query`, `spell.learn`, `spell.ignore`, `tmx.export`, `languagetool.check`, `finder.run`, `team.sync`, `team.commit`, `team.conflicts`, `script.run`, `script.slots`, `align.run`, `aligner.configure`, `wiki.import`, `med.open`.
