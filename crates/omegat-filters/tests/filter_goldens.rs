...
#[test]
fn every_committed_filter_golden_is_java_assert_eq() {
    let mut files = Vec::new();
    collect_json(&goldens_dir(), &mut files);
    assert!(
        files.len() >= 3,
        "need Text/PO/HTML Java goldens, found {}",
        files.len()
    );
    ...
    for path in &files {
        ...
        if let Some(empty_text) = spec["empty_write_text"].as_str() {
            let dest = tmp
                .path()
                .join(format!("empty-{}", src.file_name().unwrap().to_string_lossy()));
            filter
        ...
        if let Some(tr) = spec.get("translated") {
            ...
            let dest = tmp
                .path()
                .join(format!("t-{}", src.file_name().unwrap().to_string_lossy()));
            filter
