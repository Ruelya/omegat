...
fn every_filter_golden_is_strict() {
    ...
    for path in &files {
        ...
        if spec["empty_write"].as_str() == Some("preserve_source") {
            ...
            let orig = normalize_ws(&std::fs::read_to_string(&src).unwrap());
            for seg in &expected {
                assert!(
                    back.contains(seg) || orig.contains(seg) && back.contains(&seg.replace('\n', " ")),
                    "empty write dropped source {seg:?} in {}",
                    path.display()
                );
            }
            assert!(!back.is_empty(), "empty write produced empty file {}", path.display());
        }
