...
use super::helpandmanual_dialect::HelpAndManualDialect;

pub struct HelpAndManualFilter;

impl Filter for HelpAndManualFilter {
    ...
    fn parse(&self, path: &Path, _ctx: &FilterContext) -> Result<ParsedFile> {
        let dialect = HelpAndManualDialect::new();
